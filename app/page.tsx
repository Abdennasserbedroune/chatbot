...
export default function ChatPage() {
  ...
  useEffect(() => {
    const extractUserName = () => {
      ...
      for (const pattern of namePatterns) {
        ...
        if (match && match[1]) { setUserName(match[1]); break }
      }
    }
    if (messages.length > 0 && !userName) extractUserName()
  }, [messages, userName])
  ...
  useEffect(() => {
    const handleSuggestionClick = (event: CustomEvent) => setInput(event.detail)
    window.addEventListener('suggestionClick', handleSuggestionClick as EventListener)
    return () => window.removeEventListener('suggestionClick', handleSuggestionClick as EventListener)
  }, [])

  return (
    <div className="flex flex-col h-screen bg-background dark:bg-dark-background">
      <ChatHeader status={status} isLoading={isLoading} />
